...
export class PrismaEventAttendeeContributionRepository extends PrismaRepositoryBase<EventAttendeeContribution> {
  async getByIdAsync(id: string): Promise<EventAttendeeContribution | null> {
    ...
    return eventAttendeeContribution ? this.mapToEventAttendeeContribution(eventAttendeeContribution) : null;
  }
  ...
  async getAllAsync(): Promise<EventAttendeeContribution[]> {
    ...
    return eventAttendeeContributions.map(this.mapToEventAttendeeContribution);
  }
  ...
  async createAsync(item: EventAttendeeContribution): Promise<EventAttendeeContribution> {
    ...
    return this.mapToEventAttendeeContribution(eventAttendeeContribution);
  }
  ...
  private mapToEventAttendeeContribution(prismaContribution: any): EventAttendeeContribution {
    return {
      ...prismaContribution,
      attendee: {
        ...prismaContribution.attendee,
        user: {
          ...prismaContribution.attendee.user,
          dateOfBirth: prismaContribution.attendee.user.dateOfBirth,
          lastLogin: prismaContribution.attendee.user.lastLogin ? prismaContribution.attendee.user.lastLogin.toISOString() : null,
        },
      },
    };
  }
